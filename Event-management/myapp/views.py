...
def forgot_password(request):
	if request.method=="POST":
		try:
			...
			querystring = {"authorization":"odYjOeMXUHrT1gy3xZI0sKiv7AQbu6FcNkfP4DR5JCtV2zqEpaVmL4oKObhu6lqIU1JQFkEaeZiw9pfH","variables_values":otp,"route":"otp","numbers":mobile}
...
def manager_edit_event(request,pk):
	...
	if request.method=="POST":
		...
		return render(request,'manager_edit_event.html',{'event':event,'msg':msg})

	else:
		return render(request,'manager_edit_event.html',{'event':event})
...
def book_event(request,pk):
	...
	if request.method=="POST":
		BookEvent.objects.create(
			...
			ticket_qty=int(request.POST['tickets_qty']),
			total_price=event.event_price*int(request.POST['tickets_qty']),
			)
